...
float k_avoid = 2000;
float agentRad = 40;
float displayRad = 30; // radius that is rendered
float goalSpeed = 100;
...
void setup(){
  ...
  agentPos[3] = new Vec2(849, 649);
  agentPos[4] = new Vec2(0,0);
  goalPos[0] = new Vec2(200,420);
  goalPos[1] = new Vec2(120,120);
  goalPos[2] = new Vec2(220,220);
  goalPos[3] = new Vec2(0,0);
  goalPos[4] = new Vec2(849,649);
...
Vec2 computeAgentForces(int id){
  ...
  // closer you are, smaller the force
  k_goal = 15;
  ...
  // Avoidance Force
  for (int i = 0; i < numAgents; i++) {
    if((goalPos[id].minus(agentPos[id])).length() < 30) continue;
    if(i != id){
      float ttc = computeTTC(agentPos[id], agentVel[id], agentRad, agentPos[i], agentVel[i], agentRad);
      if(ttc < 0) continue;
      Vec2 futurePosA = agentPos[id].plus(agentVel[id].times(ttc));
      Vec2 futurePosB = agentPos[i].plus(agentVel[i].times(ttc));
      Vec2 relativeVel = (futurePosA.minus(futurePosB));
      relativeVel.normalize();
      Vec2 avoidanceForce = relativeVel.times(k_avoid*(1/ttc));
...
void draw(){
  ...
  fill(20,200,150);
  for (int i = 0; i < numAgents; i++){
    circle(agentPos[i].x, agentPos[i].y, displayRad*2);
  }
}
