...
void update(float dt){
  ...
  Vec2 dir = new Vec2(mouseX - pos.x,mouseY - pos.y); //Should be vector pointing from pos to MousePos; CHANGED THIS
  if (dir.length() > 0) dir.normalize();
  vel = dir.times(speed);
  if (mousePos.distanceTo(pos) < vel.times(dt).length()){
    pos = mousePos;
    vel.mul(0);
  }
...
void draw(){
  ...
  stroke(0,0,0);
  fill(255,0,0);
  circle(pos.x, pos.y,r*2); //Question: Why is it radius*2; You get the diameter
}
