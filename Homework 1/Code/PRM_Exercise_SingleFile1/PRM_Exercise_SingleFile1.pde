...
void keyPressed(){
  ...
  if (keyCode == RIGHT){
    boxTopLeft.x += 10;
    for(int i = 0; i < numNodes; i++){
      if(pointInBox(boxTopLeft, boxW, boxH, nodePos[i])) {
        buildPRM(circlePos, circleRad, boxTopLeft, boxW, boxH);
        runBFS(closestNode(startPos),closestNode(goalPos));
        break;
      }
...
int closestNode(Vec2 point){
  ...
  for(int i = 0; i < numNodes; i++) {
    float dist = (point.minus(nodePos[i])).length();
...
boolean pointInBox(Vec2 boxTopLeft, float boxW, float boxH, Vec2 pointPos){
  if(boxTopLeft.x < pointPos.x && (boxTopLeft.x + boxW) > pointPos.x){
    return true;
  }
  else if(boxTopLeft.y < pointPos.y && (boxTopLeft.y + boxH) > pointPos.y){
     return true; 
  }
  return false;
}
...
void generateRandomNodes(Vec2[] circleCenters, float[] circleRadii, Vec2 boxTopLeft, float boxW, float boxH){
  for (int i = 0; i < numNodes; i++){
    ...
    boolean insideTheRect = pointInBox(boxTopLeft, boxW, boxH, randPos);
    while (insideTheRect){
      randPos = new Vec2(random(width),random(height));
      insideTheRect = pointInBox(boxTopLeft, boxW, boxH, randPos);
    }
    nodePos[i] = randPos;
  }
}
