...
int goal = 7;
/*
ArrayList<Integer> BFSfringe = new ArrayList(); 
...
println(" Current Fring: ", BFSfringe);

while (BFSfringe.size() > 0){
  int fringeTop = 0;
  int currentNode = BFSfringe.get(fringeTop);
  BFSfringe.remove(fringeTop);
  if (currentNode == goal){
    println("Goal found!");
    break;
  }
  for (int i = 0; i < neighbors[currentNode].size(); i++){
    ...
    if (!visited[neighborNode]){
      ...
      BFSfringe.add(neighborNode);
      println("Added node", neighborNode, "to the fringe.");
      println(" Current Fringe: ", BFSfringe);
    }
  } 
}
*/
ArrayList<Integer> DFSfringe = new ArrayList(); 

println("\nBeginning DFS Search");

visited[start] = true;
DFSfringe.add(start);
println("Adding node", start, "(start) to the fringe.");
println(" Current Fring: ", DFSfringe);

// DFS

while(DFSfringe.size() > 0){
  int currentNode = DFSfringe.get(DFSfringe.size() - 1);
  DFSfringe.remove(DFSfringe.size() - 1);
  if (currentNode == goal){
    println("Goal found!");
    break;
  }
  for (int i = 0; i < neighbors[currentNode].size(); i++){
    ...
    if(!visited[neighborNode]){
      visited[neighborNode] = true;
      parent[neighborNode] = currentNode;
      DFSfringe.add(neighborNode);
      println("Added node", neighborNode, "to the fringe.");
      println(" Current Fringe: ", DFSfringe);
    }
  }
